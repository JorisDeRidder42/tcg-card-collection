...
export const AuthProvider = ({ children }) => {
  ...
  const handleAddFriend = async () => {
    ...
    try {
      ...
      if (success) {
        setMessage(`Friend request sent to ${searchResult.displayName}!`);
      }
    } finally {
      setSending(false);
    }
  ...
  const toggleSaveCard = async (card) => {
    ...
    try {
      if (isSaved) {
        ...
        await setDoc(cardRef, savedCard);
      ...
      if (firebaseUser) {
        ...
        try {
          ...
          const snapshot = await getDocs(
            collection(db, 'users', firebaseUser.uid, 'savedCards')
          );
  ...
  const value = useMemo(
    ...
    [authenticated, loading, user, profile, savedCards]
  );
