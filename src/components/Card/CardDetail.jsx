...
const CardDetail = () => {
  ...
  const { data: setCards } = useFetchList(
    card?.set?.id ? `/cards?set.id=${card.set.id}` : null
  );
  const { savedCards, toggleSaveCard } = useAuth();

  if (!card) return <h6>Card not found...</h6>;

  const saved = savedCards?.some((collection) => collection.id === card.id);
  ...
  const previousCard = currentIndex > 0 ? sortedCards[currentIndex - 1] : null;
    ...
    <Container className="py-5">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <button
          ...
          onClick={() => navigate(`/card/${previousCard.id}`)}
        >
          ← Vorige
        </button>
        ...
        <Col md={7}>
          ...
          <div className="mb-3">
            <img
              ...
              widt={30}
            />
            <span className="ms-2">{card.set?.name}</span>
          </div>
          <div className="d-flex gap-2 flex-wrap mb-4">
            <Badge bg="dark">#{card.localId}</Badge>

            <Badge bg="secondary">{card.category}</Badge>

            {card.rarity && <Badge bg="warning">{card.rarity}</Badge>}
          </div>
          {/* Pokemon */}
          {card.category === 'Pokemon' && (
            <div className="card-info-box">
              {card.hp && <p>❤️ HP: {card.hp}</p>}

              {card.types?.length > 0 && <p>Type: {card.types.join(', ')}</p>}
              <h4>Attacks</h4>
              ...
              {card.attacks?.map((attack, index) => (
                <div key={index} className="mb-3">
                  ...
                  <span className="ms-3">{attack.damage}</span>
          ...
          <p className="mt-4">
            Illustrator:
            <strong> {card.illustrator}</strong>
          </p>
